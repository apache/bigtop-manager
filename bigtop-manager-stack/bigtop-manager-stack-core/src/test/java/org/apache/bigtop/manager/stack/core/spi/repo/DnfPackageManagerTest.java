...
public class DnfPackageManagerTest {
    ...
    private final DnfPackageManager dnfPackageManager = new DnfPackageManager();
    ...
    public void testInstallPackage() {
        ...
        ShellResult result = dnfPackageManager.installPackage(List.of(packageToInstall));
        mockStatic.verify(() -> ShellExecutor.execCommand(anyList(), anyBoolean()));
        assertEquals("[/usr/bin/dnf, install, -y, testPackage]", result.getOutput());
    }

    @Test
    public void testUninstallPackage() {
        String packageToRemove = "testPackage";
        ShellResult result = dnfPackageManager.uninstallPackage(List.of(packageToRemove));
        mockStatic.verify(() -> ShellExecutor.execCommand(anyList(), anyBoolean()));
        assertEquals("[/usr/bin/dnf, remove, -y, testPackage]", result.getOutput());
    }
