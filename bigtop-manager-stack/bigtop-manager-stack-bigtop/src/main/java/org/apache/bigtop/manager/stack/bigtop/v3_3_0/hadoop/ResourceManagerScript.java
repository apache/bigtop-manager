...
public class ResourceManagerScript extends AbstractServerScript {
    ...
    public ShellResult start(Params params) {
        ...
        String cmd = MessageFormat.format("{0}/yarn --daemon start resourcemanager", hadoopParams.binDir());
        try {
    ...
    public ShellResult stop(Params params) {
        HadoopParams hadoopParams = (HadoopParams) params;
        String cmd = MessageFormat.format("{0}/yarn --daemon stop resourcemanager", hadoopParams.binDir());
        try {
            return LinuxOSUtils.sudoExecCmd(cmd, hadoopParams.user());
        } catch (Exception e) {
            throw new StackException(e);
    ...
    public ShellResult status(Params params) {
        HadoopParams hadoopParams = (HadoopParams) params;
        return LinuxOSUtils.checkProcess(hadoopParams.getResourceManagerPidFile());
    }
