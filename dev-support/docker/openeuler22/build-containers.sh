...
if ! $SKIP_BUILD; then
  ...
  if [[ -z $(docker ps -a --format "table {{.Names}}" | grep "bigtop-manager-build-oe22") ]];then
    docker run -it -d --name bigtop-manager-build-oe22 -u $(id -u):$(id -g) -v $PWD/../../../:/opt/develop/bigtop-manager/ -w /opt/develop/bigtop-manager bigtop-manager/develop:trunk-openeuler-22
  else
...
docker run -it -d -p 13306:3306 -p 15005:5005 -p 15006:5006 -p 18080:8080 --name bigtop-manager-server --hostname bigtop-manager-server --network bigtop-manager --cap-add=SYS_TIME bigtop-manager/develop:trunk-openeuler-22
...
docker exec bigtop-manager-server bash -c "mysql -e \"DROP USER 'root'@'localhost'\""

echo -e "\033[32mCreating container bigtop-manager-agent-01\033[0m"
docker run -it -d --name bigtop-manager-agent-01 --hostname bigtop-manager-agent-01 --network bigtop-manager --cap-add=SYS_TIME bigtop-manager/develop:trunk-openeuler-22
...
docker run -it -d --name bigtop-manager-agent-02 --hostname bigtop-manager-agent-02 --network bigtop-manager --cap-add=SYS_TIME bigtop-manager/develop:trunk-openeuler-22
