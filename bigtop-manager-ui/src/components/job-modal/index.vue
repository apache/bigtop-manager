...
<script setup lang="ts">
  import { computed, ComputedRef, reactive, ref, shallowRef, toRefs } from 'vue'
  ...
  type VisibleStageKeys = 'clusterId' | 'jobId' | 'stageId'
  ...
  const props = withDefaults(defineProps<Props>(), {
    visibleStageKeys: () => {
      return ['clusterId', 'jobId', 'stageId']
    }
  })

  const { jobInfo, visibleStageKeys } = toRefs(props)
  ...
  const breadcrumbs = ref<BreadcrumbItem[]>([
    {
      ...
      id: `clusterId-${jobInfo.value.clusterId}`
    }
  ])
  ...
  const apiMap = shallowRef<ApiMapItem[]>([
    {
      key: 'clusterId',
      api: getJobList
    },
  ...
  const filterApiMap = computed(() => apiMap.value.filter((v) => visibleStageKeys.value.includes(v.key)))
